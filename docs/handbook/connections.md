...
| **Vercel** (hosting + serverless functions) | the deployed `/` desk, the `/api/*` functions, and all five runtime secrets as env vars | Vercel↔GitHub git integration (auto-deploy on push to `main`) | end-user browsers | push to `main` on `erc-content-desk` |
...
- **Google Sheet / Apps Script web app** — the desk has no data: Queue, Sort, Finalize, Publish, Build, and the share page's submit all fail; nothing can be read or saved.

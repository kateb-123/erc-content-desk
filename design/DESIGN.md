...
| **Quick links** (Home's rail: Sort, Newsletter builder, the next issue, Policy Exchange, Share an item, Listserv sign-up) | one FA glyph each, accent | white 12px card, Outfit title over a muted line; Open ↗ and Copy as small outlined pills on their own line | the team's six ways out of the main page (Kate's Sep 15 list); Copy hands over one sentence with the link; the next-issue card opens the Next issue page, the issue's table with **Quick add**, the submit form that puts a new item into this issue and into the queue for Sort at once (Sep 15) |
